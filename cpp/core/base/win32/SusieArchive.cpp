	...
	std::vector<tTVPSusieFileRecord> &dest)
{
	...
	int errorcode = 0xff&GetArchiveInfo(const_cast<LPSTR>(ttstr(localname).AsStdString().c_str()), 0, 0x00, &infohandle);
	if(infohandle == NULL)
	{
	...
	unsigned long pos, unsigned long size)
{
	...
	int errorcode = 0xff & GetFile(const_cast<LPSTR>(ttstr(localname).AsStdString().c_str()), pos, (LPSTR)(void*)&memhandle,
