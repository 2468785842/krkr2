...
XP3ArchiveRepackAsyncImpl::XP3ArchiveRepackAsyncImpl()
{
	...
	CoderCompress.Coder->QueryInterface(IID_ICompressWriteCoderProperties, reinterpret_cast<void**>(&writeCoderProperties));
	CoderCompress.Coder->QueryInterface(IID_ICompressSetCoderProperties, reinterpret_cast<void**>(&setCoderProperties));
...
HRESULT XP3ArchiveRepackAsyncImpl::SetRatioInfo(const UInt64 *inSize, const UInt64 *outSize) noexcept
{
	if (OnProgress) {
...
void XP3ArchiveRepackAsyncImpl::DoConv()
{
	...
	for (unsigned int arcidx = 0; arcidx < ConvFileList.size(); ++arcidx) {
		...
		for (const auto &it : imglist) {
			...
			std::unique_ptr<tTJSBinaryStream> strImg(xp3arc->CreateStreamByIndex(&imgItem - &xp3arc->ItemVector.front()));
			std::unique_ptr<tTJSBinaryStream> strMask(xp3arc->CreateStreamByIndex(&maskItem - &xp3arc->ItemVector.front()));
