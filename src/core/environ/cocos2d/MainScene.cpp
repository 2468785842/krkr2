...
Sprite *TVPLoadCursorCUR(tTJSBinaryStream *pStream) {
	...
	for (int i = 0; i < cur_dir.size(); ++i) {
		...
		surface->initWithRawData(&pixbuf[0], pixbuf.size(), bmhdr.biWidth, bmhdr.biHeight, 0, false);
...
public:
	...
	virtual void bindBodyController(const NodeMap &allNodes) {
		_left = reinterpret_cast<ui::Button*>(allNodes.findController("left"));
...
void TVPConsoleLog(const ttstr &l, bool important) {
	...
	puts(buf);
#else
   cocos2d::log("%s", l.AsStdString().c_str());
#endif
}
