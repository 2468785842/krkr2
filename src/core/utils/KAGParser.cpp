...
const tjs_char *TVPKAGCannotOmmitFirstLabelName = TJS_W(
        "Can not ommit first label name in scenario.");
...
const tjs_char *TVPKAGSyntaxError = TJS_W(
        "Syntax error.'[' match to ']', \" match to \", 'macro' march to 'endmacro'. Notice space and newline.");
const tjs_char *TVPKAGCallStackUnderflow = TJS_W(
        "'return' is not matched to any 'call' ( 'return' is unexpected )");
const tjs_char *TVPKAGReturnLostSync = TJS_W(
        "Lost return position due to the scenario file changed.");
...
void tTJSNI_KAGParser::operator=(const tTJSNI_KAGParser &ref) {
    ...
    if (Scenario != ref.Scenario) {
        if (Scenario) Scenario->Release(), Scenario = nullptr, Lines = nullptr, CurLineStr = nullptr;
...
iTJSDispatch2 *tTJSNI_KAGParser::Store() {
    ...
    try {
        ...
        // create and assign macro dictionary
        {
            ...
            dsp->Release();
            dic->PropSet(TJS_MEMBERENSURE, TJS_W("macros"), nullptr,
                         &tmp, dic);
        ...
        // create and assign macro arguments
        {
            ...
            tTJSVariant tmp(dsp, dsp);
            dsp->Release();
            dic->PropSet(TJS_MEMBERENSURE, TJS_W("macroArgs"), nullptr,
                         &tmp, dic);
        ...
        // create call stack array and copy call stack status
        {
            ...
            tTJSVariant tmp(dsp, dsp);
            dsp->Release();
            dic->PropSet(TJS_MEMBERENSURE, TJS_W("callStack"), nullptr,
                         &tmp, dic);
            ...
            for (i = CallStack.begin(); i != CallStack.end(); i++) {
                ...
                tTJSVariant val;
                val = i->Storage;
                dic->PropSet(TJS_MEMBERENSURE, TJS_W("storage"), nullptr,
                             &val, dic);
                val = i->Label;
                dic->PropSet(TJS_MEMBERENSURE, TJS_W("label"), nullptr,
                             &val, dic);
                val = i->Offset;
                dic->PropSet(TJS_MEMBERENSURE, TJS_W("offset"), nullptr,
                             &val, dic);
                val = i->OrgLineStr;
                ...
                dic->PropSet(TJS_MEMBERENSURE, TJS_W("lineBuffer"), nullptr,
                             &val, dic);
                val = i->Pos;
                dic->PropSet(TJS_MEMBERENSURE, TJS_W("pos"), nullptr,
                             &val, dic);
                val = (tjs_int) i->LineBufferUsing;
                dic->PropSet(TJS_MEMBERENSURE, TJS_W("lineBufferUsing"), nullptr,
                             &val, dic);
                ...
                dic->PropSet(TJS_MEMBERENSURE, TJS_W("IfLevel"), nullptr,
                             &val, dic);
        ...
        val = StorageName;
        dic->PropSet(TJS_MEMBERENSURE, TJS_W("storageName"), nullptr,
                     &val, dic);
        val = StorageShortName;
        dic->PropSet(TJS_MEMBERENSURE, TJS_W("storageShortName"), nullptr,
                     &val, dic);
        ...
        val = CurLine;
        dic->PropSet(TJS_MEMBERENSURE, TJS_W("curLine"), nullptr,
                     &val, dic);
        val = CurPos;
        dic->PropSet(TJS_MEMBERENSURE, TJS_W("curPos"), nullptr,
                     &val, dic);
        ...
        val = CurLabel;
        dic->PropSet(TJS_MEMBERENSURE, TJS_W("curLabel"), nullptr,
                     &val, dic);
        ...
        val = ExcludeLevel;
        dic->PropSet(TJS_MEMBERENSURE, TJS_W("ExcludeLevel"), nullptr, &val, dic);
        val = IfLevel;
...
//---------------------------------------------------------------------------
void tTJSNI_KAGParser::StoreIntStackToDic(iTJSDispatch2 *dic, std::vector<tjs_int> &stack,
...
//---------------------------------------------------------------------------
void tTJSNI_KAGParser::StoreBoolStackToDic(iTJSDispatch2 *dic, std::vector<bool> &stack,
                                           const tjs_char *membername) {
    ttstr stack_str;
...
//	tjs_error er;
...

    {
        ...
        if (val.Type() != tvtVoid) {
            ...
            val.Clear();
            dic->PropGet(0, TJS_W("macroArgStackDepth"), nullptr,
                         &val, dic);
        ...
        if (val.Type() != tvtVoid) {
            ...
            for (tjs_int i = 0; i < count; i++) {
                ...
                Pos = val;
                dic.PropGet(0, TJS_W("lineBufferUsing"), nullptr, &val, nullptr);
                LineBufferUsing = 0 != (tjs_int) val;
                dic.PropGet(0, TJS_W("macroArgStackBase"), nullptr, &val, nullptr);
                MacroArgStackBase = (tjs_int) val;
                dic.PropGet(0, TJS_W("macroArgStackDepth"), nullptr, &val, nullptr);
                ...
                ttstr stack_str;
                dic.PropGet(0, TJS_W("ExcludeLevelStack"), nullptr, &val, nullptr);
                stack_str = val;
                RestoreIntStackFromStr(ExcludeLevelStack, stack_str);

                dic.PropGet(0, TJS_W("IfLevelExecutedStack"), nullptr, &val, nullptr);
                stack_str = val;
        ...
        val.Clear();
        dic->PropGet(0, TJS_W("storageName"), nullptr,
                     &val, dic);
        if (val.Type() != tvtVoid) StorageName = val;
        val.Clear();
        dic->PropGet(0, TJS_W("storageShortName"), nullptr,
                     &val, dic);
        if (val.Type() != tvtVoid) StorageShortName = val;
        val.Clear();
        dic->PropGet(0, TJS_W("curLabel"), nullptr,
                     &val, dic);
        ...
        val.Clear();
        dic->PropGet(0, TJS_W("ExcludeLevel"), nullptr,
                     &val, dic);
        if (val.Type() != tvtVoid) ExcludeLevel = (tjs_int) val;
        val.Clear();
        dic->PropGet(0, TJS_W("IfLevel"), nullptr,
                     &val, dic);
        ...
        val.Clear();
        dic->PropGet(0, TJS_W("macroArgStackBase"), nullptr,
                     &val, dic);
...
bool tTJSNI_KAGParser::SkipCommentOrLabel() {
    ...
    for (; CurLine < LineCount; CurLine++) {
        if (!Lines) return false; // in this loop, Lines can be nullptr when onScript does so.
...
iTJSDispatch2 *tTJSNI_KAGParser::_GetNextTag() {
    ...
    if (CurLine >= LineCount) return nullptr;
    ...
    while (true) {
        ...
        if (!LineBufferUsing && CurPos == 0)
            if (!SkipCommentOrLabel()) return nullptr;
        ...
        while (true) {
            ...
            if (CurLineStr[CurPos] == ldelim) {
                ...
                if (condition && ExcludeLevel == -1) {
                    ...
                    ismacro = TJS_SUCCEEDED(
                            Macros->PropGet(0, tagname.c_str(), nullptr, &macroval, Macros));
                    ...
                    if (tagkind == tag_emb || (ismacro && tagkind == tag_other)) {
                        ...
                        if (Owner) {
                            ...
                            tjs_error er = Owner->FuncCall(0, event_name.c_str(),
                                                           event_name.GetHint(), &res, 1, &pparam,
                                                           Owner);
            ...
            if (CurLineStr[CurPos] == TJS_W('*')) {
                ...
                if (!RecordingMacro) {
                    ...
                    if (dsp) {
                        ...
                        tTJSVariant *psrc = &src;
                        DicAssign->FuncCall(0, nullptr, nullptr, nullptr, 1, &psrc, DicObj);
                    }
                    tTJSVariant tag_val(tagname);
                    DicObj->PropSetByVS(TJS_MEMBERENSURE,
                                        __tag_name.AsVariantStringNoAddRef(), &tag_val, DicObj);
            ...
            if ((!RecordingMacro && ExcludeLevel == -1) || tagkind == tag_elsif) {
                ...
                if (entity) {
                    ...
                    if (args) {
                        tjs_char *vp = TJS_strchr(const_cast<tjs_char *>(value.c_str()),
                                                  TJS_W('|'));
...
iTJSDispatch2 *tTJSNI_KAGParser::GetMacroTopNoAddRef() const {
    if (MacroArgStackDepth == 0) return nullptr;
...
iTJSDispatch2 *TVPCreateNativeClass_KAGParser() {
    tTJSNativeClassForPlugin *classobj = TJSCreateNativeClassForPlugin(TJS_W("KAGParser"),
                                                                       Create_NI_KAGParser);
...
//	if(numparams < 0) return TJS_E_BADPARAMCOUNT;
                    ...
                    if (dsp == nullptr) {
                        if (result) result->Clear(); // return void ( not nullptr )
                    } else {
